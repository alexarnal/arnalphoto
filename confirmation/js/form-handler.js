...
/* Multi-pose aware. Derived from firstCommunion/form-handler.js:
 *   - Supports Add Pose / Remove Pose
 *   - Has a global "Bishop Class Photo" (8x10 group with Bishop Mark Seitz)
 *     callout. It's a scheduled, guaranteed item — no ack/refund logic.
...
function updateTotal() {
    ...
    if (bishopInput) {
        const q = parseInt(bishopInput.value) || 0;
        subtotal += q * 15;
    }
...
function hasAnySelection() {
    if ((parseInt(document.getElementById('bishopPhotoQty')?.value) || 0) > 0) return true;
...
function handleSubmit(e) {
    ...
    // Global Bishop Class Photo (line-item style pose)
    const bishopQty = parseInt(document.getElementById('bishopPhotoQty')?.value) || 0;
    if (bishopQty > 0) {
        poses.push({
            poseNumber: 0,
            poseType: '1-3',
            prints: { 'Bishop Class Photo (8x10)': bishopQty },
            description: 'Bishop Class Photo — group photo with Bishop Mark Seitz'
        });
    }
...
function init() {
    ...
    // Bishop photo changes
    const bp = document.getElementById('bishopPhotoQty');
    if (bp) bp.addEventListener('change', updateTotal);
